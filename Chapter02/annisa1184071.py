# -*- coding: utf-8 -*-
"""
Created on Wed Mar 31 12:26:50 2021
...
def preparation(datasetpath):
    d = pd.read_csv(datasetpath,
                    ...
                    names=['Sales', 'Order', 'Units', 'Entropy', 'Room','Suplier', 'Distribusi' ])

    d = pd.get_dummies(d, columns=[ 'Order', 'Units', 'Entropy', 'Room','Suplier', 'Distribusi'])

    encode = LabelEncoder()
    d['Sales'] = encode.fit_transform(d['Sales'])
    ...
    df_train_label = df_train_label['Sales']
    df_test_label = df_test_label['Sales']
